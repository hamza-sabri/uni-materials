...
export default function MaterialCard({ cardPhoto, cardTitle, cardRate, cardID, routeTo, submitHandler, option }: cardInterface) {
	...
	const Card = () => {
		return (
			...
			>
				<div className="add-cover" ref={addRef}>
					<span>{option}</span>
				</div>
				<img src={cardPhoto} alt="card-img" />
				<div className="rate-container" ref={starRef}>
	...
	const mouseEnterEffect = ()=>{
		if(routeTo) return;
		if(starRef.current) starRef.current.style.display ='none';
		if(addRef.current)  addRef.current.style.display  ='flex';

	}
	const mouseLeaveEffect = ()=>{
		if(routeTo) return;
		if(starRef.current) starRef.current.style.display ='flex';
		if(addRef.current)  addRef.current.style.display  ='none';
	}
